...
void loop () {
  ...
  {
    char rfmData[rf12_len];
    for (i = 0; i < rf12_len; ++i)
    {
      rfmData[i] = (char)rf12_data[i];
    }
    rfmData[i] = '\0';
    ...
    sprintf(queryString, "node=2&data=%s", rfmData);
    Serial.println();
    Serial.print("<<< REQ /temperaturelogger/?");
    Serial.print(queryString);
    Serial.print(" ");
    ether.browseUrl(PSTR("/temperaturelogger/?"), queryString, website, my_callback);
  }
  ...
  if (readStatus == 2 && readDoneTimer.poll())
  {
    ...
    Serial.print("Local data received: ");
    Serial.print(TReading);
    Serial.print("\n\n");
    
    sprintf(queryString, "node=1&data=%d", TReading);
    Serial.println();
    Serial.print("<<< REQ /temperaturelogger/?");
    Serial.print(queryString);
    Serial.print(" ");
